...
if (isset($_POST['confirmar'])) {
    ...
    mysqli_query($conexao, "ALTER TABLE bancoquest_bd.questoes AUTO_INCREMENT = 1;");
    if (!empty($conteudo) && !empty($dificuldade) && !empty($serie) && !empty($materia) && !empty($itemA) && !empty($itemB) && !empty($itemC) && !empty($itemD) && !empty($itemE) && !empty($correto)) {
        ...
        if ($result) {
            echo "<script>alert('Questão adicionada com sucesso!');window.location.href = './questao.php';</script>";
        } else {
            echo "<script>alert('Erro ao adicionar a questão!');window.location.href = './questao.php';</script>";
        }
    } else {
...
<body>
    ...
    <header>
        <div id="login-user">
            <div id="texto-user" style="display: flex;">
                ...
                <p>Olá, <?php echo $nome; ?>!<br> Seja bem-vindo <br>Matéria: <?php echo $materiaCU; ?></p>
            </div>
        </div>
        <div id="sair">
    ...
    <main>
        ...
        <form action="criar_questao.php" method="POST" onsubmit="return validateForm()">
